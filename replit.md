...
- Brand assets: the 1024px logo masters live in `assets/brand/` — `nexet-logo.png` (Nexet + Story Oracle), `nexet-agent-logo.png` (desktop agent + Creator Den) and `nexet-author-den-logo.png` (Authors Den). `pnpm --filter @workspace/scripts run logo:assets` regenerates every sized file each app serves (header mark, `favicon-32.png`, `apple-touch-icon.png`, `og-logo.png`, the den marks Nexet's room cards wear, and the agent's logo/tray/installer icon) from the master the script maps to that app — re-run it after replacing a master.
